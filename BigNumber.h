...
public:
    ...
    // Zmienna len reprezentuje aktualna długosc liczby (liczbe cyfr), a al wielkosc zaalokowanej pamieci do przechowywania cyfr liczby
    int len, al;
    ...
    explicit BigNum(long long v, int l = 2) : len(1), al(l), array_of_digits(new long long[l]) {
    ...
    // Funkcja dokonuje przenoszenia do starszych pozycji nadmiaru powstałego na skutek
    // wykonywania operacji. Jest to jedyne miejsce w całej strukturze, gdzie
    // wykonuje sie ta operacje. Parametr okresla numer pozycji, do której nalezy
    // wykonac przenoszenie nadmiaru
    void carry(int p) {
        int x = 0;
        for (; x < p || array_of_digits[x] < 0 || array_of_digits[x] >= BASE; x++) {
            Res(x + 2);
        // W razie potrzeby wykonaj pozyczke od starszej pozycji
            if (array_of_digits[x] < 0) {
                long long i = (-array_of_digits[x] - 1) / BASE + 1;
                array_of_digits[x] += i * BASE;
                array_of_digits[x + 1] -= i;
                }
            else // lub wykonaj przeniesienie powstałego nadmiaru
    ...
    BigNum& operator=(const int a) {
        ...
        len = 1;
        array_of_digits[0] = a;
        if (array_of_digits[0] >= BASE) carry(1);
        return *this;
    }
    bool operator==(const int a) {
        if(len == 1 && array_of_digits[0] == a)
            return true;
        else{
            FOR(x, 1, len){
                if(array_of_digits[x] != 0) return false;
            }
            if(array_of_digits[0] == a) return true;
            else return false;
        }
    }
    ...
    BigNum operator*(const int a) const {
        BigNum c(0);
        c = *this;
        REP(x, len) c.array_of_digits[x] *= a;
        c.carry(len);
        return c;
    }
    void operator+=(int a) {
    ...
    BigNum operator* (BigNum &a) {
        BigNum c(0, this->len + a.len);
        REP(x, a.len) {
            REP(y, len) c.array_of_digits[y + x] += this->array_of_digits[y] * a.array_of_digits[x];
            c.carry(len + x);
        }
    ...
    BigNum operator+ (const BigNum &a){
        ...
        c = *this;
        Res(c.len);
        REP(x, a.len) c.array_of_digits[x] += a.array_of_digits[x];
        carry(c.len);
        return c;
    }
    ...
    BigNum& operator /= (const BigNum &a){
        ...
        FORD(i, n - 1, 0) {
            int l = 0, r = BASE - 1;
            //wyszukiwanie binarne zeby znalezc liczbe ktora nalezy odjac podczas dzielenia
            while (l < r) {
                int m = (l + r + 1) / 2;
    ...
    std::vector<int> write(int pod) const {
        ...
        BigNum v(0, 0);
        v = *this;
        while (v.len > 1 || v.array_of_digits[0]) w.PB(v /= pod);
        return w;
    }
    // Operator przesuniecia w prawo o n pozycji
